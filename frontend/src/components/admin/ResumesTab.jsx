...
const ResumesTab = ({ rows, total, page, pageSize, onPageChange, onView, onDelete }) => (
          <div>
            <DataTable
              columns={[
                ...
                { label: 'When', render: (r) => r.Timestamp },
                {
                  label: '',
                  render: (r) => (
                    <button type="button" onClick={() => onView(r.ID)} style={{ padding: '4px 10px', borderRadius: '6px', border: '1px solid var(--color-border)', background: 'var(--color-surface)', color: 'var(--color-text)', fontSize: '12px', cursor: 'pointer' }}>
                      View
                    </button>
                  ),
                },
