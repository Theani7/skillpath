...
type Props = {
  ...
  onEditingCourseChange: (course: AdminCourse) => void;
...
const CoursesTab = ({ courses, jobRoles, newCourse, editingCourse, onNewCourseChange, onEditingCourseChange, onAdd, onSave, onCancelEdit, onEdit, onDelete }: Props) => (
  <div>
    ...
    <DataTable
      columns={[
        { label: 'Field', render: (c) => c.field as string },
        { label: 'Course', render: (c) => c.course_name as string, nowrap: true },
        { label: 'URL', render: (c) => <a href={c.course_url as string} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--color-primary)' }}>{truncate(c.course_url as string, 40)}</a>, mono: true, nowrap: true },
        {
          label: '',
          render: (c) => {
            const item = c as unknown as AdminCourse;
            return (
              <button type="button" onClick={() => onEdit(item)} style={{ padding: '4px 10px', borderRadius: '6px', border: '1px solid var(--color-border)', background: 'var(--color-surface)', color: 'var(--color-text)', fontSize: '12px', cursor: 'pointer' }}>
                Edit
              </button>
            );
          },
