...
type Analytics = {
  most_sought_role: string;
  most_common_missing_skill: string;
};
...
type Props = {
  resumes: AdminUserRow[];
  registeredUsers: RegisteredUser[];
  feedback: FeedbackItem[];
  courses: AdminCourse[];
  analytics: Analytics;
  qualityMetrics: QualityMetrics | null;
  uploadsOverTime: UploadPoint[];
  skillPathGaps: SkillGap[];
  roleDistribution: RoleDist[];
  scrapeStatus: ReactNode;
  onTriggerScrape: () => void;
};

const DashboardTab = ({ resumes, registeredUsers, feedback, courses, analytics, qualityMetrics, uploadsOverTime, skillPathGaps, roleDistribution, scrapeStatus, onTriggerScrape }: Props) => (
  <div>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '16px', marginTop: '20px' }}>
      ...
      <div className="card" style={{ padding: '24px' }}>
        ...
        {roleDistribution.length > 0 ? (
          <ResponsiveContainer width="100%" height={200}>
            <PieChart>
              <Pie data={roleDistribution} dataKey="count" nameKey="target_role" cx="50%" cy="50%" outerRadius={80} label={({ target_role, percent }: { target_role?: string; percent?: number }) => `${target_role?.slice(0, 15) || ''} ${((percent ?? 0) * 100).toFixed(0)}%`} labelLine={false} style={{ fontSize: '11px' }}>
