import {
  Layers, Target, GraduationCap, Briefcase, Compass, TrendingUp, BookOpen,
  type LucideIcon,
} from 'lucide-react';
...
export const relativeTime = (timestamp: string | number | null | undefined): string => {
  if (!timestamp) return '';
...
export const fullDate = (timestamp: string | number | null | undefined): string => {
  if (!timestamp) return '';
  const cleaned = String(timestamp).replace(' ', 'T');
...
export const detectPlatform = (url: string | undefined | null): string => {
