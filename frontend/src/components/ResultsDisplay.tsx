...
import type { AnalysisResponse, ResumeData } from '../types';
...
const ResultsDisplay = ({ data, onReset }: Props) => {
  ...
  const matchedSkills = Array.isArray((resumeInfo as ResumeData)?.skills) ? (resumeInfo as ResumeData).skills : [];
  ...
  const handleFeedbackSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.post('/api/feedback', {
        name: (resumeInfo as ResumeData)?.name || 'Anonymous',
        email: (resumeInfo as ResumeData)?.email || 'N/A',
  ...
  return (
    ...
    >
      ...
      {trends && ((trends as Record<string, unknown>).growth !== undefined || (trends as Record<string, unknown>).top_skills !== undefined) && (
